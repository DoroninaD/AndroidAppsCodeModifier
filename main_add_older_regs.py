...
#path = 'data/com.whatsapp/lib/armeabi-v7a/libcurve25519'
...
for group in groups[:10]:
    ...
    if any(big_regs[i] in first[3] for i in range(len(big_regs))): #and (str(first[2]).startswith('push') or str(first[2]).startswith('stm')):
                continue
    new_registers= utils.addRegistersToEnd(first[3])

    # добавляем в to_write (адрес, количество старых байт, новые байты) push
    to_write.append((first[0], len(first[1]) // 2,
                     utils.toLittleEndian(arm_translate.pushToCode(new_registers, first[1]))))  # добавляем новый push
    # добавлаем все pop
    for a in group[1:]:
