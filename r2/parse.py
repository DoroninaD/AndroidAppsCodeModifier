#! /usr/bin/env python
# -*- coding: utf-8 -*-
import re, r2.arm_translate, r2.utils
...
def getRxLines(lines, line, table, sp_subbed, sub_ind):
    ...
    end_reg = r2.utils.searchRe('.*(mov(eq|ne)?|(v)?ldr(b|h|sb|sh|sw)?|add)(.w)?\s{0}, .*'.format(reg),lines[start_ind+1:])
    ...
    # ... [rx]
    use_reg = r2.utils.searchRe('.*(ldr|str)(b|h|sb|sh|sw|d)?(.w)?.*\[{0}(, #\-?[0-9]+\])?'.format(reg), lines[start_ind:end_ind])
    ...
    for l in use_reg:
        ...
        if n >= 0:  # todo а что если будет sub rx, sp?
            ...
            code, is_thumb = r2.utils.getCodeFromLine(l.group())
            offset = changeOffset(n, d, table)
            #offset = d + before_reg_count * 4
            #if n >= real_reg_count * 4:
            #    offset += after_reg_count * 4
            new_instr_code = r2.arm_translate.makeLdrOrStr(instr, code, rx, reg, offset, is_thumb, l.group())
            # to_write ... [reg, #d-new_regs_count*4]

            to_write.append((r2.utils.getAddressFromLine(l.group()), len(code) // 2, r2.utils.toLittleEndian(new_instr_code)))
...
def getAllSpLinesForLow(lines, table):
    ...
    sub_add_sp_lines = r2.utils.searchRe('.*(add|sub)(.w)?\s*sp(, sp)?, #[0-9]+', opcodes)
   ...
   #use_sp_lines = list(filter(None,[re.search('.*(ldr|str)(b|h|sb|sh|d)?(.w)?.*\[sp, #[0-9]+\].*', line) for line in lines]))
    use_sp_lines = r2.utils.searchRe('.*(ldr|str)(b|h|sb|sh|d)?(.w)?.*\[sp, #[0-9]+\].*', opcodes)
    #todo
    ...
    for l in use_sp_lines:
        ...
        if b-a >= 0:
            ...
            new_instr_code = r2.arm_translate.makeLdrOrStr(instr,code , rx, 'sp', offset, is_thumb, l.group())
    ...
    add_sp_to_reg = r2.utils.searchRe('.*(add(.w)?|mov)\s*(r[0-9]|r10|r11|r12), sp(, #[1-9]+)?.*', opcodes)
    #todo
